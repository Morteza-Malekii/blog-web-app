                ...
                <div class="rounded custom-box-shadow rounded p-4 p-sm-5">
					...
					<form action="{{ route('register')}}" class="mt-4" method="POST">
						...
						<div class="mb-3">
							...
							<input type="text" name="name" value="{{old('name')}}" class="form-control @error('name') border-danger

                            @enderror" id="exampleInputName"  placeholder="نام و نام خانوادگی">
                            @error('name')
						...
						<div class="mb-3">
							...
							<input type="email" name="email" value="{{old('email')}}" class="form-control @error('email') border-danger

                            @enderror" id="exampleInputEmail1"  placeholder="ایمیل">
                            @error('email')
							<small id="emailHelp" class="form-text text-danger">{{$message}}</small>
                            @enderror
						</div>
						...
						<div class="mb-3">
							...
							<input type="password" name="password" class="form-control @error('password') border-danger
